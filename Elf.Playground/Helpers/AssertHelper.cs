...
namespace Elf.Playground.Helpers
{
        ...
        public static void SequenceEqual<T>(IEnumerable<T> expected, IEnumerable<T> actual, Func<T, bool, bool> filter)
        {
            ...
            if (!allMatch)
            {
                ...
                throw new AssertionException(String.Format(
                    "Sequences differ at position '{0}'. Expected '{1}', actual '{2}'.",
                    commonLen,
                    expected.Count() <= commonLen ? "N/A" : expected.ElementAt(commonLen).ToInvariantString(),
                    actual.Count() <= commonLen ? "N/A" : actual.ElementAt(commonLen).ToInvariantString()));
            }
        }
